...
- The creation of the message will be successful if and only if the messageText is not blank, is under 255 characters, and postedBy refers to a real, existing user. If successful, the response body should contain a JSON of the message, including its message_id. The response status should be 200, which is the default. The new message should be persisted to the database.
- If the creation of the message is not successful, the response status should be 400. (Client error) */
    public Message createNewMessageService(Message message){
        Optional<Account> messageInAccountOptional = accountRepository.findById(message.getPostedBy());
...
As a user, I should be able to submit a PATCH request on the endpoint PATCH localhost:8080/messages/{message_id}. The request body should contain a new messageText values to replace the message identified by message_id.
...
which is retrieved from the database. It is expected for the list to simply be empty if there are no messages. The response status should always be 200, which is the default.*/
    public List<Message> getMessagesOfaUser(int accountId){
        Optional<List<Message>> listOptional = messageRepository.getPostedBy(accountId);
